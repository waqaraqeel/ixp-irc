...
#     prefixes_announced = bgp_get_announced_routes(sdx, 'X')

    final_policy = (
            (match(dstip=IPPrefix('110.0.0.0/16')) >> sdx.fwd(participant.peers['A'])) + 
            (match(dstip=IPPrefix('120.0.0.0/16')) >> sdx.fwd(participant.peers['B'])) +
            (match(dstip=IPPrefix('10.0.0.0/16')) >> sdx.fwd(participant.phys_ports[0]))
            )
